...
client.on('messageReactionAdd',async  (reaction  , user ) => {
  ...
  if (reaction.message.partial) await reaction.message.fetch();
    let rolefetch = await react.fetchrr(client, reaction.message.guild.id ,reaction.message.id , reaction.emoji.id);
    if(!rolefetch) return;
...
client.on('messageReactionRemove',async  (reaction, user) => {
 ...
 if(member.roles.cache.has(rolefetch.roleid)){
  ...
  console.log(`Role on ${reaction.emoji.name} has been REMOVED`)
  }
});
```
